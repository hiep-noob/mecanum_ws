        ...
        marker_count_(0)
{
    ...
    map_update_interval_ = tmp_sec*10e9; 
    ...
    sst_  = node->create_publisher<nav_msgs::msg::OccupancyGrid>("map", rclcpp::QoS(1).reliable().transient_local());
    sstm_ = node->create_publisher<nav_msgs::msg::MapMetaData>("map_metadata", rclcpp::QoS(1).reliable().transient_local());
    marker_publisher_ = node->create_publisher<visualization_msgs::msg::MarkerArray>(
                        "visualization_marker_array", rclcpp::QoS(10));    

    scan_filter_sub_ = node->create_subscription<sensor_msgs::msg::LaserScan>(
                      "scan", rclcpp::SensorDataQoS(), std::bind(&SlamKarto::laserCallback, this, std::placeholders::_1));
...
double SlamKarto::getYaw(tf2::Transform& t)
{
...
void SlamKarto::publishLoop()
{
    ...
    tmp_tf_stamped.header.stamp = node->get_clock()->now() + rclcpp::Duration( std::chrono::nanoseconds(int(transform_tolerance_*10e9)) );
    tmp_tf_stamped.transform = tf2::toMsg(map_to_odom_);
    tf2B_->sendTransform(tmp_tf_stamped);
}

karto::LaserRangeFinder* SlamKarto::getLaser(const sensor_msgs::msg::LaserScan::SharedPtr scan)
{
  if(lasers_.find(scan->header.frame_id) == lasers_.end())
  {
    ...
    try
    {
        ...
        laser_pose_msg = tf2_buffer_.lookupTransform(base_frame_, scan->header.frame_id, tf2::TimePointZero);
        tf2::fromMsg(laser_pose_msg.transform, laser_pose);          
    }
    ...
    // Check if laser is inverted based on Z axis orientation
    tf2::Vector3 laser_z_axis = laser_pose.getBasis().getColumn(2);
    bool inverse = lasers_inverted_[scan->header.frame_id] = laser_z_axis.z() <= 0;
...
bool SlamKarto::getOdomPose(karto::Pose2& karto_pose, const rclcpp::Time& t)
{
    ...
    try
    {
        ...
        odom_pose_msg = tf2_buffer_.lookupTransform(odom_frame_, base_frame_, tf2::TimePointZero);
        tf2::fromMsg(odom_pose_msg.transform, odom_pose);   
    }
    catch(tf2::TransformException& e)
    {
...
void SlamKarto::publishGraphVisualization()
{
  ...
  edge.header.stamp = node->get_clock()->now();
  edge.action = 0; 
  edge.ns = "karto";
  edge.id = 0;
  edge.type = 4; 
...
void SlamKarto::laserCallback(const sensor_msgs::msg::LaserScan::SharedPtr scan)
{
  laser_count_++;
...
bool SlamKarto::updateMap()
{
  ...
  map_.map.header.stamp = node->get_clock()->now();
                   ...
                   karto::Pose2& karto_pose)
{
    ...
    if((processed = mapper_->Process(range_scan)))
    {
        ...
        tf2::Transform map_to_base(q, tf2::Vector3(corrected_pose.GetX(),
                                              corrected_pose.GetY(),
                                              0.0));
        ...
        try
        {
            ...
            odom_to_base_msg = tf2_buffer_.lookupTransform(odom_frame_, base_frame_, tf2::TimePointZero);
...
bool SlamKarto::mapCallback(const nav_msgs::srv::GetMap::Request::SharedPtr req,
                             nav_msgs::srv::GetMap::Response::SharedPtr res)
